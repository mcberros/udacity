...
class SignUpHandler(render.Handler):
  ...
  def post(self):
    ...
    if err_msg['error_username'] == "" and err_msg['error_password'] == "" and err_msg['error_verify'] == "" and err_msg['error_email'] == "":
      ...
      self.render_front(in_username, 
                        "", 
                        "", 
                        err_msg['error_username'], 
                        err_msg['error_password'], 
                        err_msg['error_verify'], 
                        in_email,err_msg['error_email'])
...
app = webapp2.WSGIApplication([(DOCUMENT_ROOT + '/signup', SignUpHandler), 
                            (DOCUMENT_ROOT + '/login', LoginHandler), 
                            (DOCUMENT_ROOT + '/logout', LogoutHandler), 
                            (DOCUMENT_ROOT + '/_edit' + PAGE_RE, EditHandler), 
                            (DOCUMENT_ROOT + '/_history' + PAGE_RE, HistoryHandler), 
                            (DOCUMENT_ROOT + PAGE_RE, ViewHandler)], 
                            debug = True)
